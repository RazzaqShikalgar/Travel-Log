...
route.get("/filter",async(req,res)=>{
  //North
const url = 'https://nomad-list-cities.p.rapidapi.com/nomad-list/asia?size=20&page=1&sort=desc&sort_by=overall_score';
...
//africa
const url1 = 'https://nomad-list-cities.p.rapidapi.com/nomad-list/africa?size=20&page=1&sort=desc&sort_by=overall_score';
...
// console.log(lodging.data[0].result_type);  //lodging data
  ...
  res.render("advisor",{place,hotels:hotel.data,attraction:attraction.data,location:location_id.data[0],image:location_img.data,freeimages,lodging:lodging.data});
 //Hotels Best
});
...
res.render("avail",{ names: req.data.name,
  images: req.data.image,hoteld:hoteldetails});
});
...
route.post("/categoryupload", async (req, res, next) => {
